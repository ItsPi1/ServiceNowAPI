...
Describe "Update-SNConfigurationItem" {
    Context "Parameters" {
        BeforeAll {
            $commandInfo = Get-Command Update-SNConfigurationItem
        }
    ...
    Context "Test Validate Script for Fields parameter" {
        ...
        It 'Should not accepts hashtables with Keys that begin with sys_' {
            ...
            { Update-SNConfigurationItem -Name 'XXXX' -Fields @{ sys_AnInvalidKey = 'Value1' } -APIMSubscriptionKey 'XXXXX' } | Should -Throw
            { Update-SNConfigurationItem -Name 'XXXX' -Fields @{ sys_AnInvalidKey2 = 'Value2' } -APIMSubscriptionKey 'XXXXX' } | Should -Throw
        }

        It 'Should accept hashtables with Keys that do not begin with sys_' {
            { Update-SNConfigurationItem -Name 'XXXX' -Fields @{ TestKey = 'Pineapple' } -APIMSubscriptionKey 'XXXXX' } | Should -Not -Throw
            { Update-SNConfigurationItem -Name 'XXXX' -Fields @{ AValidKey = 'Value' } -APIMSubscriptionKey 'XXXXX' } | Should -Not -Throw
        }
    }
}