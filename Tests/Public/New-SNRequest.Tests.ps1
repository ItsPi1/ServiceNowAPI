...
Describe "New-SNRequest" {
    BeforeAll {
        $commandInfo = Get-Command New-SNRequest
    }
    Context "Parameters" {
    ...
    Context "Test Validate Script for Fields parameter" {
        ...
        It 'Should not accepts hashtables with Keys that begin with sys_' {
            ...
            { New-SNRequest -Fields @{ sys_AnInvalidKey = 'Value1' } -APIMSubscriptionKey 'XXXXX' } | Should -Throw
            { New-SNRequest -Fields @{ sys_AnInvalidKey2 = 'Value2' } -APIMSubscriptionKey 'XXXXX' } | Should -Throw
        }

        It 'Should accept hashtables with Keys that do not begin with sys_' {
            { New-SNRequest -Fields @{ TestKey = 'Pineapple' } -APIMSubscriptionKey 'XXXXX' } | Should -Not -Throw
            { New-SNRequest -Fields @{ AValidKey = 'Value' } -APIMSubscriptionKey 'XXXXX' } | Should -Not -Throw
        }
    }
}