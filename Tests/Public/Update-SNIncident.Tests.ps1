...
Describe "Update-SNIncident" {
    BeforeAll {
        $commandInfo = Get-Command Update-SNIncident
    }
    Context "Parameters" {
    ...
    Context "Test Validate Script for Fields parameter" {
        ...
        It 'Should not accepts hashtables with Keys that begin with sys_' {
            ...
            { Update-SNIncident -IncidentID 'INCXXXX' -Fields @{ sys_AnInvalidKey = 'Value1' } -APIMSubscriptionKey 'XXXXX' } | Should -Throw
            { Update-SNIncident -IncidentID 'INCXXXX' -Fields @{ sys_AnInvalidKey2 = 'Value2' } -APIMSubscriptionKey 'XXXXX' } | Should -Throw
        }

        It 'Should accept hashtables with Keys that do not begin with sys_' {
            { Update-SNIncident -IncidentID 'INCXXXX' -Fields @{ TestKey = 'Pineapple' } -APIMSubscriptionKey 'XXXXX' } | Should -Not -Throw
            { Update-SNIncident -IncidentID 'INCXXXX' -Fields @{ AValidKey = 'Value' } -APIMSubscriptionKey 'XXXXX' } | Should -Not -Throw
        }

        # TODO: Add test for INCID validate script
    }
}